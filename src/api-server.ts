...
export class ApiServer implements Api {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `Loading ${vmIds.length} VM${vmIds.length === 1 ? "" : "s"}...`,
      );
  ...
  async listVms(): Promise<VmInfo[]> {
    const promises = [...this.vms.entries()].map(async ([id, vm]) => {
      const completed =
        vm instanceof CreateVm ? await vm.complete() : undefined;
  ...
  async listImages(): Promise<ImageInfo[]> {
    ...
    for (const [id, image] of this.images.entries()) {
      const completed =
        image instanceof CreateImage ? await image.complete() : undefined;
  ...
  async removeImage(id: string): Promise<void> {
    ...
    if (info.status === "downloading") {
      if (!(image instanceof CreateImage)) {
        throw new Error(
          `Image is marked downloading but is not an active CreateImage job: ${info.name}`,
        );
      }
    ...
    const linkedVms = (await this.listVms()).filter(
      (vm) => vm.baseImageId === resolvedId,
    );
    if (linkedVms.length > 0) {
      throw new Error(
        `Cannot remove image while VMs reference it: ${linkedVms.map((vm) => vm.name).join(", ")}`,
      );
    }
  ...
  private async validateCreateVmInput(input: CreateVmInput): Promise<void> {
    ...
    const existingVm = (await this.listVms()).find(
      (vm) => vm.name === input.name,
    );
    if (existingVm) {
  ...
  private async validateCreateImageParams(
    ...
    const existingImage = (await this.listImages()).find(
      (image) => image.name === params.name,
    );
    if (existingImage) {
  ...
  private async normalizeCreateVmInput(input: CreateVmInput): Promise<
    Omit<CreateVmInput, "user" | "baseImageId"> & {
      user: string;
      baseImageId: Id;
    }
  > {
  ...
  private async resolveCreateVmParams(
    input: Omit<CreateVmInput, "user" | "baseImageId"> & {
      user: string;
      baseImageId: Id;
    },
  ...
  private async requireLoadedVm(id: Id): Promise<LoadVm> {
    ...
    if (isVmCreateInProgress(info.status)) {
      throw new Error(
        `Cannot operate on VM while creation is in progress: ${info.name}`,
      );
    }
  ...
  private async resolveBaseImage(value: string): Promise<ImageInfo> {
    ...
    if (nameMatches.length > 1) {
      const readyMatches = nameMatches.filter(
      ...
      throw new Error(
        `Base image name must resolve to one ready image: ${value}`,
      );
    }
