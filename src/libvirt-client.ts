...
export class LibvirtClient {
  constructor(
    private readonly uri = process.env.LIBVIRT_URI ?? DEFAULT_LIBVIRT_URI,
  ) {}
  ...
  getGuestInterfaces(name: string): GuestInterfaceInfo[] {
    ...
    if (!Array.isArray(payload.return)) {
      throw new Error(
        `Guest agent response did not include an interface list for ${name}`,
      );
    }

    return payload.return.map((value, index) =>
      parseGuestInterfaceInfo(value, name, index),
    );
  }
...
function isMissingNetworkError(message: string): boolean {
  ...
  return (
    ...
    normalized.includes("failed to get network")
  );
}
...
function parseGuestInterfaceInfo(
  value: unknown,
  vmName: string,
  index: number,
): GuestInterfaceInfo {
  ...
  if (!("name" in value) || typeof value.name !== "string") {
    throw new Error(
      `Guest interface ${index} for ${vmName} did not include a valid name`,
    );
  }
...
function parseGuestIpAddresses(
  value: unknown,
  vmName: string,
  ...
  if (!Array.isArray(value)) {
    throw new Error(
      `Guest interface ${interfaceName} for ${vmName} had invalid ip-addresses`,
    );
  }
  ...
  return value.map((entry, index) => {
    if (typeof entry !== "object" || entry === null) {
      throw new Error(
        `Guest IP address ${index} for ${interfaceName} on ${vmName} was not an object`,
      );
    }

    const ipAddress =
      "ip-address" in entry && typeof entry["ip-address"] === "string"
        ? entry["ip-address"]
        : undefined;
    const ipAddressType =
      "ip-address-type" in entry &&
      (entry["ip-address-type"] === "ipv4" ||
        entry["ip-address-type"] === "ipv6")
        ? entry["ip-address-type"]
        : undefined;
    const prefix =
      "prefix" in entry && typeof entry.prefix === "number"
        ? entry.prefix
        : undefined;

    if (!ipAddress || !ipAddressType || prefix === undefined) {
      throw new Error(
        `Guest IP address ${index} for ${interfaceName} on ${vmName} was incomplete`,
      );
    }
