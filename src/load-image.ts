  ...
  async getInfo(): Promise<ImageInfo> {
    ...
    if (metadata) {
      return {
        ...
        hash: metadata.hash,
        sizeBytes: await this.getFileSize(
          await this.dataDir.getImagePath(this.id),
        ),
        progress: 1,
      };
    }
    ...
    if (hasDownload) {
      return {
        ...
        sizeBytes: await this.getFileSize(
          await this.dataDir.getImageDownloadPath(this.id),
        ),
        progress: 0,
        error: request
          ? undefined
          : "Interrupted download is missing request.json",
      };
    }
