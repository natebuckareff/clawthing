...
export class TablePrinter {
  ...
  addRow(values: TableValue[]): void {
    if (values.length !== this.headers.length) {
      throw new Error(
        `Expected ${this.headers.length} columns, received ${values.length}`,
      );
    }
  ...
  render(): string {
    const widths = this.headers.map((header, index) =>
      Math.max(
        header.length,
        ...this.rows.map((row) => row[index]?.length ?? 0),
      ),
    );
    const lines = [this.headers, ...this.rows].map((row) =>
      formatRow(row, widths, this.columnSpacing),
    );
    return lines.join("\n");
  }
}
...
function formatRow(
  ...
  return values
    .map((value, index) => {
      ...
      return value.padEnd(widths[index] ?? 0, " ");
    })
    .join(separator);
}
